...
const { getWoocommerceRootAndChildBySKU } = require('./functions/woocommerce_actions');
...
fs.readFile(__dirname + '/xml_example_2.xml', function(err, data) {
    parser.parseString(data, async function (err, json_file) {
        ...
        for (let i = 0; i < product_results.length; i++) {
          ...
          console.log('woocommerce_product',product_results[i].reference, woocommerce_product);
